...
, box_(nullptr)
{
    box_ = gtk_box_new(GTK_ORIENTATION_VERTICAL, FALSE);
    gtk_container_add(GTK_CONTAINER(m_widget), box_);

    GtkWidget *gl_area = gtk_gl_area_new();
    gtk_widget_set_hexpand(gl_area, TRUE);
    gtk_widget_set_vexpand(gl_area, TRUE);
    gtk_container_add(GTK_CONTAINER(box_), gl_area);
    g_signal_connect(gl_area, "realize", G_CALLBACK(realize), NULL);
    g_signal_connect(gl_area, "unrealize", G_CALLBACK(unrealize), NULL);
    g_signal_connect(gl_area, "render", G_CALLBACK(render), NULL);

    GtkWidget *controls = gtk_box_new(GTK_ORIENTATION_VERTICAL, FALSE);
    gtk_container_add(GTK_CONTAINER(box_), controls);
    ...
    Bind(wxEVT_PAINT, &GLPanel::OnPaint, this, wxID_ANY);
...
void GLPanel::OnSize(wxSizeEvent &e)
{
    if (box_)
    {
        ...
        gtk_widget_set_size_request(box_, e.GetSize().GetWidth(), e.GetSize().GetHeight());
        gtk_container_resize_children(GTK_CONTAINER(box_));
    }
}
...
void GLPanel::OnPaint(wxPaintEvent& e)
{
    ...
    dc.DrawLine(rc.GetTopLeft(), rc.GetBottomRight());
    dc.DrawLine(rc.GetBottomLeft(), rc.GetRightTop());
}
