...
// Constructors
    ...
    wxImageThumbnailItem(const wxBitmap& thumbnailBitmap, const wxString& filename) :
        wxThumbnailItem(filename),
        m_cachedBitmap(thumbnailBitmap) {}
...
class wxPreloadImageThumbnailItem : public wxImageThumbnailItem
{
    DECLARE_CLASS(wxPreloadImageThumbnailItem)
public:
    wxPreloadImageThumbnailItem(const wxBitmap& thumbnailBitmap, const wxString &gUUID, const wxString& filename) :
        wxImageThumbnailItem(thumbnailBitmap, filename), m_groupUUID(gUUID) {}
    ~wxPreloadImageThumbnailItem() {}

    /// Load the thumbnail
    bool Load(wxThumbnailCtrl* ctrl, bool forceLoad) override { return true; }
    wxString &GetGroupUUID() { return m_groupUUID; }
    const wxString &GetGroupUUID() const { return m_groupUUID; }
    void SetThumbnailBitmap(const wxBitmap& thumbnailBitmap) { m_cachedBitmap = thumbnailBitmap; }

private:
    wxString m_groupUUID;
};
...
wxDECLARE_EVENT(wxEVT_COMMAND_THUMBNAIL_MIDDLE_CLICK, wxThumbnailEvent);
