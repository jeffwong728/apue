...
wxDEFINE_EVENT(wxEVT_COMMAND_THUMBNAIL_MIDDLE_CLICK, wxThumbnailEvent);
...
bool wxThumbnailItem::DrawBackground(wxDC& dc, wxThumbnailCtrl* ctrl, const wxRect& rect, const wxRect& imageRect, int style, int WXUNUSED(index))
{
    ...
    if (!filename.IsEmpty() && (ctrl->GetWindowStyle() & wxTH_TEXT_LABEL))
    {
        ...
        fRect.y = rect.y + rect.height - margin - textH;
    ...
    {
        wxPen dottedPen(ctrl->GetFocusRectColour(), 1, wxPENSTYLE_DOT);
        dc.SetPen(dottedPen);
...
IMPLEMENT_CLASS(wxPreloadImageThumbnailItem, wxImageThumbnailItem)
