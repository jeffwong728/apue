...
cmake --no-warn-unused-cli `
    ...
    -DwxBUILD_VENDOR:STRING= `
    -DwxBUILD_TOOLKIT:STRING=gtk3 `
    -DwxBUILD_MONOLITHIC:BOOL=TRUE `
    -DwxUSE_REGEX:STRING=builtin `
    -DwxUSE_ZLIB:STRING=sys `
    -DwxUSE_EXPAT:STRING=sys `
    -DwxUSE_LIBJPEG:STRING=sys `
    -DwxUSE_LIBPNG:STRING=sys `
    -DwxUSE_LIBTIFF:STRING=sys `
    -DwxUSE_LIBLZMA:STRING=sys `
    -DwxUSE_STL:BOOL=TRUE `
    -DwxUSE_STD_CONTAINERS:BOOL=TRUE `
    -DwxBUILD_DISABLE_PLATFORM_LIB_DIR:BOOL=TRUE `
    -DwxBUILD_SHARED:BOOL=TRUE `
    -DwxBUILD_PRECOMP:BOOL=FALSE `
    -DwxBUILD_SAMPLES:STRING=SOME `
    -DHAVE_STD_UNORDERED_MAP:BOOL=TRUE `
    -DHAVE_STD_UNORDERED_SET:BOOL=TRUE `
    -DwxUSE_MEMORY_TRACING:BOOL=TRUE `
    -DwxUSE_NATIVE_DATAVIEWCTRL:BOOL=FALSE `
