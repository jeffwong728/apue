...
def extract_ng_BBTs(bbtLogPath, efBBTPath):
    ...
    with open(efBBTPath, "w") as w:
        for trTag in trTags[1:]:
            ...
            statusString = unicode(statusTag.string)
            if statusString!=u"PASS" and statusString!=u"RERUN_PASS" and statusString!=u"SKIP":
                if not firstLine:
                    w.write("\n")
                w.write(unicode(trTag.contents[1].string))     
                firstLine = False

def extract_ng_RTests(rtestLogPath, efRTestPath):
    ...
    with open(efRTestPath, "w") as w:
        for trTag in trTags[2:]:
            ...
            statusString = unicode(statusTag.string)
            if statusString==u"FAIL" or statusString==u"ERROR" or statusString==u"INCONCLUSIVE":
                if not firstLine:
                    w.write("\n")
