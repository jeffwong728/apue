...
class H5DBHelper : private boost::noncopyable
{
public:
    H5DBHelper() = delete;
...
private:
    ...
    template<typename T> static T Load_impl(const H5::Group &g, const std::string &n, std::true_type, const T &d = T());
...
public:
    static void Save(const H5::Group &g, const std::string &n, const cv::Scalar &c);
    static void SetAttribute(const H5::H5Object &o, const std::string &n, const cv::Scalar &c);
    ...
    template<typename T> static T Load(const H5::Group &g, const std::string &n, const T &d = T());
    ...
    static std::vector<cv::String> GetSpamProjects(const cv::String &dbPath);
};

template<typename T>
void H5DBHelper::Save_impl(const H5::Group &g, const std::string &n, const T &v, std::true_type)
{
    if (g.nameExists(n))
    {
...
template<typename T>
T H5DBHelper::Load_impl(const H5::Group &g, const std::string &n, std::true_type, const T &d)
{
    if (g.nameExists(n))
    {
...
template<typename T>
void H5DBHelper::SetAttribute_impl(const H5::H5Object &o, const std::string &n, const T &v, std::true_type)
{
    if (o.attrExists(n))
    {
        o.removeAttr(n);
...
template<typename T>
T H5DBHelper::GetAttribute_impl(const H5::H5Object &g, const std::string &n, std::true_type)
{
    T val = T();
    if (g.attrExists(n))
    {
...
template<typename T>
void H5DBHelper::Save(const H5::Group &g, const std::string &n, const T &v)
{
    Save_impl(g, n, v, std::is_pod<T>());
}

template<typename T>
void H5DBHelper::Save(const H5::Group &g, const std::string &n, const std::vector<T> &seq)
{
    if (g.nameExists(n))
    {
        H5Ldelete(g.getId(), n.data(), H5P_DEFAULT);
...
void H5DBHelper::Save(const H5::Group &g, const std::string &n, const std::vector<std::array<T, Dim>> &points)
{
    if (g.nameExists(n))
    {
        H5Ldelete(g.getId(), n.data(), H5P_DEFAULT);
...
template<> cv::Scalar H5DBHelper::Load<cv::Scalar>(const H5::Group &g, const std::string &n, const cv::Scalar &d);
template<> std::string H5DBHelper::GetAttribute<std::string>(const H5::H5Object &o, const std::string &n);
template<> cv::Scalar H5DBHelper::GetAttribute<cv::Scalar>(const H5::H5Object &o, const std::string &n);

template<typename T>
T H5DBHelper::Load(const H5::Group &g, const std::string &n, const T &d)
{
    return Load_impl<T>(g, n, std::is_pod<T>(), d);
}

template<typename T>
bool H5DBHelper::Load(const H5::Group &g, const std::string &n, std::vector<T> &seq)
{
    seq.clear();
...
template<typename T, std::size_t Dim>
bool H5DBHelper::Load(const H5::Group &g, const std::string &n, std::vector<std::array<T, Dim>> &points)
{
    points.clear();
    if (g.nameExists(n))
    {
...
template<typename T>
void H5DBHelper::SetAttribute(const H5::H5Object &o, const std::string &n, const T &v)
{
    SetAttribute_impl(o, n, v, std::is_pod<T>());
}

template<typename T>
T H5DBHelper::GetAttribute(const H5::H5Object &o, const std::string &n)
{
    return GetAttribute_impl<T>(o, n, std::is_pod<T>());
}

template<typename T, std::size_t Dim>
void H5DBHelper::Save(const H5::Group &g, const std::string &n, const std::array<T, Dim> &arr)
{
    if (g.nameExists(n))
    {
        H5Ldelete(g.getId(), n.data(), H5P_DEFAULT);
...
template<typename T, std::size_t Dim>
bool H5DBHelper::Load(const H5::Group &g, const std::string &n, std::array<T, Dim> &arr)
{
    if (g.nameExists(n))
    {
        H5::DataSet dataset = g.openDataSet(n);
