...
struct ShapeTopLayerScaner
{
    ...
    ShapeTopLayerScaner(const ShapeTemplate *const shapeTmpl, const RunLength *const r, const float s, const float g)
        : tmpl(shapeTmpl)
        , roi(r)
        , minScore(s)
        , greediness(g)
...
void ShapeCandidateScaner<false>::operator()(const tbb::blocked_range<int>& r) const
{
    ...
    for (int c = r.begin(); c != r.end(); ++c)
    {
        ...
        float maxScore = -1.f;
        int bestTmplIndex = 0;
        const ScalableIntSequence &tmplIndices = upperTmplDatas[candidate.mindex].mindices;
...
void DeformCandidateScaner<false>::operator()(const tbb::blocked_range<int>& r) const
{
    ...
    const auto &upperTmplDatas = tmpl->pyramid_tmpl_datas_[layer + 1].tmplDatas;
    vcl::Vec8i vecMinContrast(minContrast*minContrast * 64);
    ScalablePointSequence maxDeforms, curDeforms;
...
int ShapeTemplate::matchShapeTemplate(const cv::Mat &img,
    ...
    float &score)
{
    ...
    std::tie(numSearchRuns, searchRuns) = getSearchRegion(smo.searchRgn, nTopRows, nTopCols);
...
int ShapeTemplate::CreateTemplate(const ShapeTmplCreateData &createData)
{
    destroyData();
    ...
    low_contrast_  = createData.lowContrast;
    high_contrast_ = createData.highContrast;
...
void ShapeTemplate::DrawInstance(cv::Mat &img, const cv::Point2f &pos, const float angle, const DrawTmplOptions &opts) const
{
    if (!pyramid_tmpl_datas_.empty())
    {
        ...
        if (!lsd.tmplDatas.empty())
        {
            ...
            cv::Mat rotMat = cv::getRotationMatrix2D(cv::Point2f(0.f, 0.f), shtd.angle - angle, 1.0);
            ...
            for (const cv::Point2d &ept : transEdgePoints)
            {
                ...
                if (!oib(pt))
                {
                    ...
                    {
                        img.at<cv::Vec3b>(pt) = cv::Vec3b(0x00, 0x00, 0xFF);
                    }
                    else
                    {
                        img.at<uchar>(pt) = 0xFF;
                    }
                }
            }
        }
...
int ShapeTemplate::createShapeTemplate(const ShapeTmplCreateData &createData)
{
    ...
    cv::Mat tmplImg;
    cv::medianBlur(createData.img, tmplImg, 5);
    ...
    for (int l = 0; l < createData.pyramidLevel; ++l)
    {
        ...
        for (double ang = 0; ang < createData.angleExtent; ang += angleStep)
        {
            ...
            affMat = HomoMat2d::Rotate(affMat, deg, cv::Point2d());
            ...
            {
                err_msg_ = levelStr + "template region out of range";
            ...
            ScalablePointSet *edgeLocs = static_cast<ScalablePointSet *>(&ptd.edgeLocs);
            ...
            if (l == createData.pyramidLevel-1)
            {
                constexpr int simdSize = 8;
